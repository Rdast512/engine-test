...
private:
    ...
    void pickPhysicalDevice() {
        ...
        std::cout << termcolor::green << "Using physical device: " << physicalDevice.getProperties().deviceName << std::endl;
        ...
        for (const auto& qfp : ret) {
            std::cout << termcolor::green << "Queue family: " << qfp.queueFamilyProperties.queueCount
                      << " graphics: " << ((qfp.queueFamilyProperties.queueFlags & vk::QueueFlagBits::eGraphics) != static_cast<vk::QueueFlags>(0))
                        << " compute: " << ((qfp.queueFamilyProperties.queueFlags & vk::QueueFlagBits::eCompute) != static_cast<vk::QueueFlags>(0))
                        << " transfer: " << ((qfp.queueFamilyProperties.queueFlags & vk::QueueFlagBits::eTransfer) != static_cast<vk::QueueFlags>(0))
    ...
    void createLogicalDevice() {
        ...
        vk::StructureChain<vk::PhysicalDeviceFeatures2, vk::PhysicalDeviceVulkan11Features , vk::PhysicalDeviceVulkan13Features, vk::PhysicalDeviceExtendedDynamicStateFeaturesEXT, vk::PhysicalDeviceShaderFloatControls2FeaturesKHR> featureChain = {
        ...
        if (graphicsIndex == presentIndex) {
            std::cout << termcolor::green << "\nUsing single queue for graphics and present : "<< graphicsIndex << std::endl;
        } else {
        ...
        if (transferIndex != queueFamilyProperties.size() && transferIndex != graphicsIndex) {
            ...
            std::cout << termcolor::yellow << "No dedicated transfer queue found, will use graphics queue for transfers" << std::endl;
        }
        ...
        std::cout << termcolor::green << "Using graphics queue: " << graphicsIndex << "\nUsing present queue: " << presentIndex
        << "\nUsing transfer queue: " << (transferIndex != UINT32_MAX ? std::to_string(transferIndex) : "N/A") << std::endl;
    ...
    void createCommandPool() {
        ...
        vk::CommandPoolCreateInfo transferPoolInfo{ .flags = vk::CommandPoolCreateFlagBits::eResetCommandBuffer, .queueFamilyIndex = transferIndex };
    ...
    void createBuffer(vk::DeviceSize size, vk::BufferUsageFlags usage, vk::MemoryPropertyFlags properties,
        ...
        vk::BufferCreateInfo bufferInfo{ .size = size, .usage = usage, .sharingMode = vk::SharingMode::eConcurrent,
        .queueFamilyIndexCount = static_cast<uint32_t>(queueFamilyIndices.size()),
            .pQueueFamilyIndices = queueFamilyIndices.data() };
    ...
    void createDescriptorPool() {
        ...
        vk::DescriptorPoolCreateInfo poolInfo{ .flags = vk::DescriptorPoolCreateFlagBits::eFreeDescriptorSet,
            .maxSets = MAX_FRAMES_IN_FLIGHT, .poolSizeCount = 1, .pPoolSizes = &poolSize };
    ...
    void createDescriptorSets() {
        ...
        for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
            ...
            vk::WriteDescriptorSet descriptorWrite{ .dstSet = descriptorSets[i], .dstBinding = 0, .dstArrayElement = 0,
                .descriptorCount = 1, .descriptorType = vk::DescriptorType::eUniformBuffer, .pBufferInfo = &bufferInfo };
    ...
    void createTextureImage() {
        ...
        transitionImageLayout(transferCommandBuffer[0], textureImage, vk::ImageLayout::eUndefined, vk::ImageLayout::eTransferDstOptimal);

        copyBufferToImage(stagingBuffer, textureImage, static_cast<uint32_t>(texWidth), static_cast<uint32_t>(texHeight));

        transitionImageLayout(transferCommandBuffer[0],textureImage, vk::ImageLayout::eTransferDstOptimal, vk::ImageLayout::eShaderReadOnlyOptimal);
    }

    void copyBufferToImage(const vk::raii::Buffer& buffer, vk::raii::Image& image, uint32_t width, uint32_t height) {
        transferCommandBuffer[0].begin({.flags = vk::CommandBufferUsageFlagBits::eOneTimeSubmit});
        ...
        transferCommandBuffer[0].copyBufferToImage(buffer, image, vk::ImageLayout::eTransferDstOptimal, {region});
    ...
    void createImage(uint32_t width, uint32_t height, vk::Format format, vk::ImageTiling tiling, vk::ImageUsageFlags usage, vk::MemoryPropertyFlags properties, vk::raii::Image& image, vk::raii::DeviceMemory& imageMemory) {
        vk::ImageCreateInfo imageInfo{ .imageType = vk::ImageType::e2D, .format = format,
                                      ...
                                      .usage = usage, .sharingMode = vk::SharingMode::eConcurrent, .queueFamilyIndexCount = static_cast<uint32_t>(queueFamilyIndices.size()),
                                        .pQueueFamilyIndices = queueFamilyIndices.data()};
    ...
    void endSingleTimeCommandsTransfer(vk::raii::CommandBuffer& commandBuffer) {
        commandBuffer.end();
    ...
    void createCommandBuffers() {
        ...
        if (transferIndex != UINT32_MAX && transferIndex != graphicsIndex) {
            vk::CommandBufferAllocateInfo transferAllocInfo{ .commandPool = transferCommandPool, .level = vk::CommandBufferLevel::ePrimary,
        ...
        std::cout << termcolor::green << "Transfer command buffers allocated: " << transferCommandBuffer.size() << std::endl;
    }
    ...
    void transitionImageLayout(
        ...
        if (oldLayout == vk::ImageLayout::eUndefined && newLayout == vk::ImageLayout::eTransferDstOptimal) {
            ...
            dstAccessMask = vk::AccessFlagBits2::eNone; // Present has no specific access type
        ...
        // Add other common transitions here...
        else {
            // If the transition is not a common one we handle automatically, fall back to a generic, safe barrier.
            // Or, if overrides are provided, this will be skipped.
            if (!srcStageMaskOverride.has_value()) { // A simple check to see if we should throw or use a safe default
                 srcAccessMask = vk::AccessFlagBits2::eMemoryWrite;
                 dstAccessMask = vk::AccessFlagBits2::eMemoryRead | vk::AccessFlagBits2::eMemoryWrite;
                 srcStageMask = vk::PipelineStageFlagBits2::eAllCommands;
                 dstStageMask = vk::PipelineStageFlagBits2::eAllCommands;
                // For production code, you might want to throw an exception here if an unknown transition
                // is not explicitly handled by the caller via overrides.
    ...
    void cleanup() {
        ...
        // Explicitly reset buffers before their memory gets destroyed
        ...
        // Clear uniform buffers and their memory
        uniformBuffers.clear();
        uniformBuffersMemory.clear();
        uniformBuffersMapped.clear();

        // Clear other Vulkan objects
        descriptorSets.clear();
